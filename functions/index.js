...
// 5. Monitor Oysta Vehicles (V.14.8.4)
...
exports.monitorOystaVehicles = functions.pubsub.schedule('every 2 minutes').onRun(async (context) => {
    ...
    try {
        ...
        // BUG FIX (V.14.8.4): No consultamos emergencias porque no se hace seguimiento en BG.
        ...
        // BUG FIX (V.14.8.4): Salimos si no hay preventivos activos.
        ...
        // BUG FIX (V.14.8.4): Solo recursos de PREVENTIVOS (no de emergencias).
        ...
        // BUG FIX (V.14.8.4): usa activeIntDocs (correcto) en lugar de activeIntsSnap (no definido).
        ...
        for (const intDoc of activeIntDocs) {
            ...
            for (const rid of intAssignedIds) {
                ...
                // V.14.8.4: Detección más robusta. Si Oysta no envía speed, intentamos detectar por status o movimiento
                ...
                // Log de diagnóstico temporal (V.14.8.4): logueamos las claves del primer objeto que veamos
                if (!diagnosticLogged) {
                    await db.collection("oysta_logs").add({
                ...
                // Reset de arribo agresivo (V.14.8.4: resetea siempre > 250m para evitar botes)
                if (vs.hasArrived && distToDest > 0.25) {
                    vs.hasArrived = false;
                    vehicleStateChanged = true;
                }

                // --- NUEVA LÓGICA V.14.8.4 (Reglas de Casos A, B y C) ---
        ...
        if (oystaVehicleCount > 0) {
            await db.collection("oysta_logs").add({
                ...
                usuario: "Oysta (BG)",
                tipo: "Oysta",
                // BUG FIX (V.14.8.4): activeIntsSnap no existía; usa activeIntDocs.length
...
// V.14.8.4: Geocodificación inversa robusta con fallback a link de Google Maps
