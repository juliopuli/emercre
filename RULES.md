...
### Rama de Despliegue
- **SIEMPRE** usar la rama **`main`** para todos los commits y pushes. Esta es la rama configurada en GitHub Pages y la única que puede disparar el despliegue automático.
- **NUNCA** hacer push a `master` directamente. Si se trabaja localmente en `master`, se debe hacer merge/push a `main` antes de considerar el trabajo terminado.
- El comando estándar para publicar cambios es: `git push origin <rama-local>:main`
